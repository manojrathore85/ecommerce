...
@section('content')
<div class="container">
    <form method="POST" action="{{url($flate && $flate->copy == false ? '/flate/'.$flate->id : '/flate') }}" id="formflate" name="formflate">
        @csrf
        ...
        <div class="mb-3 row">
            <label for="name" class="col-md-4 col-form-label">Opening Balance</label>
            <div class="col-md-8">
                <div class="input-group">
                    <input type="text" name="op_balance" id="op_balance" class="form-control numericonly" value="{{$flate ? $flate->op_balance : old('op_balance')}}"/>
                    <select class="input-group-text" id="drcr" name="drcr">
                        <option value="DR" @selected($flate ? $flate->drcr == 'DR' : old('drcr') == 'DR')>DR</opttion>
                        <option value="CR" @selected($flate ? $flate->drcr == 'CR' : old('drcr') == 'CR')>CR</opttion>
                    </select>
                    <span class="text-danger">@error('op_balance'){{$message}}@enderror</span>
                </div>   
            </div>
        </div>
        ...
        <x-input type="text" name="maintenance_amount" id="maintenance_amount" label="Maintenance Amount" value="{{$flate ? $flate->maintenance_amount : old('maintenance_amount')}}" errorname="maintenance_amount" class="numericonly" />
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$flate->status}}</td>
                <td>
                    <form action="{{url('/flate/'.$flate->id)}}" method="POST">@csrf @if($flate) @method('delete')@endif
                        <a href="{{url('/flate/').'/'.$flate->id.'/copy'}}" class="btn btn-sm btn-primary">Copy</a>
                        <a href="{{url('/flate/').'/'.$flate->id .'/edit'}}" class="btn btn-sm btn-primary">Edit</a>
