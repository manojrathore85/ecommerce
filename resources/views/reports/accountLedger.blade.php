...
<div class="card">
    <div class="card-header">
        ...
        ?>
        <x-input type="date" name="fromdate" id="fromdate" label="From Date" :value="$fromdate" errorname="fromdate"/>
        <x-input type="date" name="todate" id="todate" label="To Date" :value="$todate" errorname="todate"/>
        <x-select name="account_id" id="account_id" label="Account" :options="$accounts" :selected="$account_id" errorname="account_id"/>
    ...
    <table id="table-journal" class="table table-bordered table-striped">
            ...
            <tbody>
               ...
               // echo "<pre>";  print_r($data); echo "</pre>";              
                ...
                <tr>                    
                    <td>{{$key > 0 && $row->no === $data[$key - 1]->no ? ' ' : $rowNumber ++}}</td>
                    <td>{{($key > 0 && $row->no === $data[$key - 1]->no) || $row->date == 0 ? ' ' : \Carbon\Carbon::parse($row->date)->format('d-m-Y') }}</td>
                    <td>@if($key > 0 && $row->no === $data[$key - 1]->no ) 
                        '-'; 
                        @elseif($key > 0)
                        <a href="{{url("voucher/{$row->voucher_id}/edit")}}">{{$row->no}}</a>
                        @endif
                    </td>
                    <td>{{$key > 0 && $row->no === $data[$key - 1]->no ? ' ' : $row->type}}</td>                   
                    <td>@if($key > 0){{$row->name}}@endif</td>
